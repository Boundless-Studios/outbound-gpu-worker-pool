...
artifact: plan
status: active
source_issue: BOU-3174
branch: extract-library
---

# Extract the worker pool into a provider-neutral library

**Goal:** turn the private slice-1 implementation in `magic-video-editor`
(`src/magic_video_editor/worker_pool/`, migration `006_worker_pool.sql`, and its tests) into
this public, provider-neutral Python package, then have `magic-video-editor` consume it.

**Source of truth for behavior:** the private implementation on branch
`bou-3174-gpu-worker-pool-slice-1` (PR 28), whose tests encode every acceptance criterion.
Port the behavior and the tests; do not redesign. `docs/design.md` is the architecture.
...
`postgresql://postgres:test@127.0.0.1:15432/worker_pool`; on the maintainer's laptop use the
tunnel on port 25432 and database `magic_video_editor` — any database works because every
...
### Task 4 — magic-video-editor consumes the library (separate repo, separate PR)
- Add the dependency, delete `magic_video_editor/worker_pool/`, keep `capability_id` etc.
  on `video_jobs` only if the product needs them for its own UI; otherwise submit pool jobs
  through the library's store and map completion back into the video job. Out of scope for
  this repository's first PR; tracked as its own bead.
